...
# Display bloxels
def display(bloxels, high_resolution=False, charset=None, true_color=False, random_chars=False, block=None):
  if high_resolution:
    even = bloxels[0::2]
    odd = bloxels[1::2]
    ...
    for i, row in enumerate(even):
      for j, col in enumerate(row):
        try:
          print("{}{}▄".format(ANSI.bg_true(*col), ANSI.fg_true(*odd[i][j])), end='')
        except IndexError:
          pass
      print(ANSI.end)
